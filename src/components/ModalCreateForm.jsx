...
const ModalCreateForm = ({
  onFormSubmit, dateInfo, navigateToCreate, isLoading,
}) => {
    ...
    <form onSubmit={onSubmit}>
      ...
      <div className="parent" style={{ marginBottom: '1rem' }}>
        <>
          ...
          <span style={{ marginLeft: '0.5rem' }}>{formatRoomInfo(dateInfo)}</span>
        </>
      </div>
      <Controller
        control={control}
      ...
      <div>
        <>
          <Button
            ...
            iconDescription="Add"
          >
            { isLoading ? <Loader type="ThreeDots" color="#fff" height={10} /> : 'Pievienot' }
          </Button>
          <Button
            kind="ghost"
            size="sm"
